...
-- ai_usage is written/read only via the service-role client, which bypasses
-- RLS — enabling RLS with no policies blocks anon/authenticated entirely.
-- SECURITY DEFINER helpers get a pinned search_path so they cannot be
-- hijacked via a malicious schema, and anon cannot execute them.
-- ============================================================
